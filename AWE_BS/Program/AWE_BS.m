...
function [PIP,smoothed_2,threshold,...
    ...
    AWE_BS(imName,imDir,normPix,minThreshold)
...
if normPix==1
    ...
    for u=1:size(GreyIm,1)
        ...
        %X(u,:)=scale(u)/scale(end); %create a matrix of coefficients for normalisation
    end
...
%&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
   [threshold,PIP] = makePip_BS(GreyIm,minThreshold);
   smCoef = 0.1;
   res=0.005;
   possWCap=1;
   flag=NaN;

%------------------------------Claus for minThreshold cutting too much data
if length(PIP)>20  %when minThreshold is set high, PIP can often be lower than 20 in length, resulting in an error 
...
finalThresh(1,1:2) = [1,0];
            ...
            if ~isempty(firstPeaks)
                ...
                if ~isempty(secondPeaks) 
                    if ~isempty(firstTroughs)
                    firstTroughs(firstTroughs(:,1) < min(secondPeaks(:,1)),:) = [];
                    end
                    if ~isempty(secondTroughs)
