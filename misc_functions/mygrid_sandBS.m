...
%      [image_data,vlat,vlon] = mygrid_sandBS(imDir,region,iopt)
%
...
%       imDir  = the string directory where the 1min Topo image file is 
%                located. 
%	output:
%		image_data  
...
%     
%       13/05/2015 NUI Galway, Brian Scanlon
%       edited by Brian Scanlon to work with image file topo_18.1.img
%       ftp://topex.ucsd.edu/pub/global_topo_1min/   is where it can be
%       downloaded
%   ftp://topex.ucsd.edu/pub/global_topo_1min/README_V18.1.txt <<Im info
function  [image_data,vlat,vlon] = mygrid_sandBS(imDir,region,iopt)

%DatabasesDir = '/export/home/grunion/cdh/airforce/sandwell.d';
DatabasesDir = imDir;
if ~ischar(imDir)
    errordlg('the input imDir is not a string');
end
...
for i = 1:length(wlon);
	...
	% Open the data file
	fid = fopen([DatabasesDir '/topo_18.1.img'], 'r','s');
	if (fid < 0)
		errordlg(['Could not open database: ' DatabasesDir '/topo_18.1.img'],'Error');
	end
